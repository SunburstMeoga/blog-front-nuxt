<template>
  <div class="share-post-col">
    ...
    <div class="trending-sidebar-slider position-relative">
      <span @click="sharePostPrev" class="prev slick-arrow" style="display: block"><i
      ...
      <slider :settings="trendingSidebarSlide" ref="sharePost">
        <div class="most-share-post-items">
          <template v-for="(data, index) in sharePosts.slice(0, 5)">
            <share-card :class="[darkClass && 'item' in darkClass ? darkClass.item : '']" :count="index + 1" :datas="data"
              :key="index" />
          </template>
        </div>
        <div class="most-share-post-items">
      ...
      <span @click="sharePostNext" class="next slick-arrow" style="display: block"><i
